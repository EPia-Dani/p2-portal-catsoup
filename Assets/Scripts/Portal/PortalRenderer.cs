...
namespace Portal {
	...
	public void SetFrameSkipInterval(int interval)
	{
		...
		private void OnBeginCameraRendering(ScriptableRenderContext context, Camera currentCamera) {
			...
			// Exclude pair portal from occlusion checks (prevents false occlusion when portals are close)
			Transform excludeTransform = pair != null ? pair.transform : null;
			if (!PortalVisibility.IsVisible(mainCamera, _view.SurfaceRenderer, excludeTransform)) return;
